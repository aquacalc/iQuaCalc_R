    ...
    fluidPage(
      ...
      tabsetPanel(
        ...
        tabPanel(title = 'L/W Gallery',
                 ...
                 fluidRow(
                   ...
                   column(width = 4,
                          ...
                          wellPanel(style = 'padding: 5px;',
                            h4(HTML('Choose a species from the list on the right for a <em>rough</em>
                               estimate of expected weight from a measured length, but <em>without</em> 
                               the prediction interval.')),
                            br(),
                            h5(HTML('(If you have length & weight data, use the \"Upload L/W Data\" tab
                               for an analysis <em>with</em> the prediction interval).'))
                          )
                   ),
                   ...
                   column(width = 8,
                          ...
                          ), # END 'top' fluidRow, with rb group & plot
                          ...
                          ) # END 'bottom' fluidRow, with paper info
                     
                   ) # END column with 'top' & 'bottom' fluidRows
                   
                 ), # END fluidRow #1/2 for 'L/W Gallery'
                 ...
                 ) # END fluidRow #2/2 for 'L/W Gallery'
                 
        ),  # END tabPanel(title = 'L/W Gallery'
                   ...
                   column(width = 4,
                          ...
                          fluidRow(
                            ...
                            column(width = 6,
                                   ...
                                   # hidden(radioButtons(ns('rb_length_metric_options'),
                                   #                     'Length Units',
                                   #                     choices = c('TL (BL)', 'CL 1', 'CL 2'),
                                   #                     selected = 'TL (BL)',
                                   #                     inline = T))
...
lwUploadModule <- function(input, output, session, st) {
  ...
  calc_monodon_data <- reactive({
    ...
    gallery_lw_data$monodon_lw_data <- data.frame(length  = l,
                                                  weight  = w,
                                                  count   = c,
                                                  count_2 = c_whole_kg)
  })
  ...
  calc_semisulcatus_data <- reactive({
    ...
    gallery_lw_data$semisulcatus_lw_data <- data.frame(length  = l,
                                                       weight  = w,
                                                       count   = c,
                                                       count_2 = c_whole_kg)
  })
  ...
  calc_atlantic_data <- reactive({
    ...
    gallery_lw_data$atlantic_lw_data <- data.frame(length  = l,
                                                   weight  = w)
                                               # count   = c,
                                               # count_2 = c_whole_kg)
    # species = 'L. vannamei')
  ...
  calc_chinook_data <- reactive({
    ...
    gallery_lw_data$chinook_lw_data <- data.frame(length  = l,
                                                  weight  = w)
  })
  ...
  calc_coho_data <- reactive({
    ...
    gallery_lw_data$coho_lw_data <- data.frame(length  = l,
                                               weight  = w)
  })
  ...
  calc_pink_data <- reactive({
    ...
    gallery_lw_data$pink_lw_data <- data.frame(length  = l,
                                               weight  = w)
  })
  ...
  gallery_lw_data_to_plot_shrimp <- reactive({
    ...
    cat('\nYADDA: ', input$rb_gallery_spp_shrimp, '\n')
    ...
    if('L. vannamei' == input$rb_gallery_spp_shrimp) {
      ...
      cat('\n=============================\n')
      print(sapply(spp_to_plot, class))
      print(sapply(calc_vann_data, class))
      cat('\n-----------------------------\n')
      print(sapply(spp_to_plot, class))
      cat('\n\n-+++++++++++++++++++++++-\n\n\n')
